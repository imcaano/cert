...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check if MetaMask address already exists (excluding current student)
    $stmt = $pdo->prepare('SELECT id FROM students WHERE metamask_address = ? AND id != ?');
    $stmt->execute([$metamask_address, $student_id]);
    if ($stmt->fetch()) {
        $errors[] = 'This MetaMask address is already registered by another student';
    }
    
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare('UPDATE students SET name = ?, metamask_address = ?, email = ? WHERE id = ?');
            if ($stmt->execute([$name, $metamask_address, $email, $student_id])) {
                $success_message = 'Student updated successfully!';
                // Update the student data for display
                $student['name'] = $name;
                $student['metamask_address'] = $metamask_address;
                $student['email'] = $email;
            } else {
...
<head>
    ...
    <title>Edit Student - CertChain Admin</title>
            ...
            <div class="col-md-9 col-lg-10 px-0">
                <div class="main-content p-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <div>
                            <a href="view_student_certificates.php?id=<?php echo $student['id']; ?>" class="btn btn-info me-2">
                                ...
                                <form method="POST" action="">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="metamask_address" name="metamask_address" 
                                               value="<?php echo htmlspecialchars($student['metamask_address']); ?>" 
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-text">This is the unique identifier for this student</div>
                                    </div>
